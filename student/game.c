...
GameState copy(GameState *game_state) { //Creates a copy of *game_state 
    GameState new_game;
    ...
    for (int r = 0; r < new_game.rows; r++) {
        ...
        memmove(new_game.board[r], game_state->board[r], new_game.columns * sizeof(char));
    }
    return new_game;
}

int recursive_best_score(GameState *game_state, int depth) { 
  // Simulates one turn for each possible option in the current *game_state
    ...
    for (int i = 0; i < 5; i++) {
        ...
        free_game_state(&copy_gs); // Free allocated memory
    }
    return best_score;
}

int show_best_move(GameState *game_state) {
  // Determines the best move possible according to the best score
    int best_move = NONE;
    int best_score = game_state->score;
    int moves[] = {MOVE_LEFT, MOVE_RIGHT, ROTATE_CW, ROTATE_CCW, NONE};
